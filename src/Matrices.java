...
public class Matrices {
    ...
    public static class Vector {
        ...
        public int dotProduct(Vector other) {
            ...
            int result = 0;
            for(int i = 0; i < this.items.length; i++) {
                result += this.items[i] * otherItems[i];
            }
            return result;
        }
        ...
        public double length() {
            double sumPowItem = 0;
            for(int i = 0; i < this.items.length; i++) {
                sumPowItem += this.items[i] * this.items[i];
            }
            return Math.sqrt(sumPowItem);
        }
    ...
    public static class Matrix {
        ...
        /**
         * Добавлен конструктор для преобразования массива в объект Matrix
         * @param rows
         */
        ...
        public Matrix scalarMultiply(int scalar) {
            ...
            for(int iRow = 0; iRow < this.rows.length; iRow++) {
                int[] resultRow = new int[this.rows[iRow].length]; //Для результирующей строки
                for(int iCol = 0; iCol < this.rows[iRow].length; iCol++) {
                    resultRow[iCol] = this.rows[iRow][iCol] * scalar;
                }
                result[iRow] = resultRow;
            }
            return new Matrix(result);
        ...
        public Matrix transpose() {
            // Для упрощения предположим что все строки матрицы одинаковой длины(ранее не декларировалось)
            Matrix result = new Matrix(this.nCols,this.nRows); // Строки и столбцы меняем местами
            for(int iRow = 0; iRow < this.rows.length; iRow++) {
                for(int iCol = 0; iCol < this.rows[iRow].length; iCol++) {
                    result.rows[iCol][iRow] = this.rows[iRow][iCol];
                }
            }
            return result;
        }
        ...
        private static int determinant(int[][] matrix) {
            // Для упрощения предположим что все строки матрицы одинаковой длины
            ...
            for (int i = 0; i < matrix[0].length; i++) {
                ...
                // В цикле убираем нулевую строку и i-тый элемент
                int[][] smallerMatrix = new int[matrix.length - 1][matrix.length - 1];
                //todo(); // Заполнение smallerMatrix нужными значениями из matrix
                {
                    for (int iRow = 1; iRow < matrix.length; iRow++ ) { // нулевую строку текущего массива не берём
                        int offsetCol = 0 ;
                ...
                // КОНЕЦ //todo(); // Заполнение smallerMatrix нужными значениями из matrix
